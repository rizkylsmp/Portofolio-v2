...
const AdminLogin = ({ onAuthenticated }: Props) => {
  const serverAuth = isServerAuthMode();
  const [isSetup] = useState(() => !isSetupDone()); // true = first-time setup mode
  ...
  const [lockoutRemaining, setLockoutRemaining] = useState(() => getLockoutInfo().remainingMs);
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-text-muted text-xs mt-6">
          {serverAuth
            ? "Admin menggunakan backend server dan token sesi sementara."
